...
fs.readdirSync(path.join(__dirname, 'filters'))
  ...
  .forEach(x => {
    ...
    metamocha.addFilter(
      new FilterModule({ runtimeTopology: Environment.displayName || environmentName })
    );
  });
...
function start() {
  if (process.env.MONGODB_VERSION) {
    console.log('Using MongoDB version from environment');
    return Promise.resolve({
      version: process.env.MONGODB_VERSION.split('.'),
      ssl: process.env.SSL
    }).then(_start);
  }
  const versionCheckManager = new ServerManager();
  return versionCheckManager.discover().then(_start);
}

function _start(serverInfo) {
